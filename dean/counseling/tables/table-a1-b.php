...
$email = $_SESSION['useremail'];
	...
	if ($sem == 'All') {
		...
		$sql1 = "SELECT * FROM crecords1 WHERE sy>='$sy1' AND sy<='$sy2' AND status = 'Pending' AND type = 'acad' and uid = '$email' GROUP BY idnum order by student_lastname asc";
		
	}
	else 	{
		$sql1 = "SELECT * FROM crecords1 WHERE sy>='$sy1' AND sy<='$sy2' AND status = 'Pending' AND type = 'gui' AND type = 'acad' and uid = '$email' GROUP BY idnum order by student_lastname asc";
