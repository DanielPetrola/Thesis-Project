...
$email = $_SESSION['useremail'];
	...
	if ($sem == 'All') {
		...
		$sql1 = "SELECT * FROM crecords1 WHERE sy>='$sy1' AND sy<='$sy2' AND status = 'To be acknowledged' AND uid = '$email' GROUP BY idnum order by student_lastname asc";
		
	}
	else 	{
		$sql1 = "SELECT * FROM crecords1 WHERE sy>='$sy1' AND sy<='$sy2' AND status = 'To be acknowledged' AND sem = '$sem' AND uid = '$email' GROUP BY idnum order by student_lastname asc";
		...
		?>                  
			         ...
			         <td> 
			         	<form action="counseling-p1-a2.php" method="post">
