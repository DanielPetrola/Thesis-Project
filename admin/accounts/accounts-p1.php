...
<link rel="stylesheet" type="text/css" href="https://cdn.datatables.net/datetime/1.1.0/css/dataTables.dateTime.min.css">
    ...
    top: 7%;
    left: 1%;">
			<button class="btn btn-success btn-sm" onclick="location.href='accounts-new.php'">NEW <i class="white fa fa-plus"></i></button></div>
			<!-- searchbox -->
			<?php
				if(isset($_POST['query']))
				{
				    ...
				    $query = "SELECT * FROM accounts WHERE CONCAT(`idnum`, `lastname`, `firstname`, `middleinit`) LIKE '%".$valueToSearch."%'";
				    $search_result = filterTable($query);				    
				}
				 else { 
				    $query = "SELECT * FROM accounts ORDER BY lastname ASC";
				    $search_result = filterTable($query);
				}
...
<table class="table" id="table1">
  <thead class="thead" style="background-color: #7e1414;">
    <tr>
      ...
      <th scope="col" style="color: #fff;">POSITION</th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td width="20%;" > <?php echo ucwords($row['role']);?></td>
      <td width="20%;"> <?php echo ucwords($row['position']);?></td>
      <td width="10%;"> 
    ...
    <?php $cnt=$cnt+1; } 
			...
			<!--	<table id="customers2" >
                    <tr>
                      ...
                      <th width="30%;"><a href="accounts-p1-2.php">NAME <i class="white fa fa-sort"></i></a></th>
                      <th width="20%;">ROLE</th>
