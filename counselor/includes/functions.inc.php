...
function createCRecord($conn, $idnum, $lastname, $firstname, $middleinitial, $course, $yearlevel, $date1, $timestart, $timeend, $venue, $reason1, $attendance, $appearance, $attitude, $difficulty, $corrective, $date2, $comments, $commentss, $type, $uln, $ufn, $chk, $gender, $age, $year, $sem, $uid, $status) {
	$sql = "INSERT INTO crecords1 (idnum, student_lastname, student_firstname, middleinitial, student_course, student_yrlvl, date, time_start, time_end, venue, reason, attendance, appearance, attitude, difficulty, corrective, date_next, c_comments, s_comments, type, uln, ufn, chk, gender, age, sy, sem, uid, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
	$stmt = mysqli_stmt_init($conn);
	if (!mysqli_stmt_prepare($stmt, $sql)) {
		...
		mysqli_stmt_bind_param($stmt, "sssssssssssssssssssssssssssss", $idnum, $lastname, $firstname, $middleinitial, $course, $yearlevel, $date1, $timestart, $timeend, $venue, $reason1, $attendance, $appearance, $attitude, $difficulty, $corrective, $date2, $comments, $commentss, $type, $uln, $ufn, $chk, $gender, $age, $year, $sem, $uid, $status);
		mysqli_stmt_execute($stmt);
