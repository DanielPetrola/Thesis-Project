...
$email = $_SESSION['useremail'];
	...
	if ($sem == 'All') {
		...
		$sql1 = "SELECT * FROM crecords2 WHERE sy>='$sy1' AND sy<='$sy2' AND status = 'Finished' AND uid = '$email' GROUP BY idnum order by student_lastname asc";
		
	}
	else 	{
		$sql1 = "SELECT * FROM crecords2 WHERE sy>='$sy1' AND sy<='$sy2' AND status = 'Finished' AND sem = '$sem' AND uid = '$email' GROUP BY idnum order by student_lastname asc";
